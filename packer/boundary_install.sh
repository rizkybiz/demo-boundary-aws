...
[Service]
ExecStart=/usr/local/bin/${NAME} server -config /opt/boundary/${NAME}-${TYPE}.hcl
User=boundary
Group=boundary
...
sudo chown boundary:boundary /opt/boundary/${NAME}-${TYPE}.hcl
...
if [ "${TYPE}" = "controller" ]; then
  sudo /usr/local/bin/boundary database init -skip-auth-method-creation -skip-host-resources-creation -skip-scopes-creation -skip-target-creation -config /opt/boundary/${NAME}-${TYPE}.hcl || true
fi
